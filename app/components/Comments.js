  ...
  return (
    <div className="pixel-card p-6 md:p-8 flex flex-col h-full bg-surface w-full max-h-[850px]">
      ...
      <form onSubmit={handleSubmit} className="flex flex-col gap-4 mb-8">
        ...
        <button type="submit" className="pixel-btn w-full justify-center text-center mt-2 flex items-center gap-2">
            ...
            <div key={comment.id} className="border-[3px] border-border p-4 bg-section-alt shadow-[4px_4px_0_rgba(0,0,0,0.05)]">
              <div className="flex justify-between items-start mb-2">
                <div className="flex items-center gap-3">
                  {comment.photo ? (
                    <img src={comment.photo} alt={comment.name} className="w-8 h-8 rounded-full border-2 border-primary object-cover" />
                  ) : (
                ...
                <span className="text-text-secondary text-xs">{comment.date}</span>
              </div>
